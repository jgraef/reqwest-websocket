...
use futures_util::{ready, select, FutureExt, Sink, SinkExt, Stream, StreamExt};
...
impl WebSocket {
    pub async fn new(request: Request, protocols: &Vec<String>) -> Result<Self, Error> {
        ...
        wasm_bindgen_futures::spawn_local(
            run_websocket(websocket, ack_tx, outgoing_rx, incoming_tx).instrument(task_span),
        );
...
async fn run_websocket(
    ...
    ack_tx: oneshot::Sender<Result<String, Error>>,
    ...
    let on_error_callback = {
        ...
        Closure::<dyn FnMut(ErrorEvent)>::new(move |event: ErrorEvent| {
            let _ = error_tx.send(event.into());
        })
    };
    ...
    select! {
        _ = open_rx => {
            let _ = ack_tx.send(Ok(websocket.protocol()));
            run_socket = true;
        }
        _ = &mut close_rx => {
            let _ = ack_tx.send(Err(Error::ConnectionFailed));
        }
